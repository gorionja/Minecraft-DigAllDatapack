...
# シルクタッチ付きのツールを使用した場合
execute if entity @s[nbt={SelectedItem:{tag:{Enchantments:[{lvl:1s,id:"minecraft:silk_touch"}]}}}] run function orion.digall:run_silk

# シルクタッチ付きのツールを使用していない場合
execute unless entity @s[nbt={SelectedItem:{tag:{Enchantments:[{lvl:1s,id:"minecraft:silk_touch"}]}}}] run function orion.digall:run
